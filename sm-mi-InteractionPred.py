...
#fpredict=open('prediction-0.8-1.txt','w')
...
def AppendRecordsAndEncode(new_items, input_type, seq_f, id_no_f):
    ...
    append_new_id.to_excel(id_no_f, index=None, header=None)   
...
#    append_sm_encode.to_csv('temp.csv',index=False)
...
#    append_mir_encode.to_csv('temp.csv',index=False)
...
new_smiles = 'C1=CC(=C(C=C1CCN)O)OCC1CCOOC'
exist_smiles = CheckSmilesExist(new_smiles, default_files['sm_smiles'])

new_mirnas = 'UGAGGUAGUAGGUUGUAUAAUUGG'
exist_mirnas = CheckMirnasExist(new_mirnas, default_files['mir_seq'])

if exist_smiles:
    print("SM File Not Updated, SMILES already exist!")
    new_smiles = []
if exist_mirnas:
    print("MiRNA File Not Updated, Seq. already exist!")
    new_mirnas = []
...
if not exist_smiles or not exist_mirnas:
    print('ReRun Needed')
